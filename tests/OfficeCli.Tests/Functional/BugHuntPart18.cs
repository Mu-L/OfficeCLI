...
public class BugHuntPart18 : IDisposable
{
    ...
    // ==================== BUG #5: PPTX shape Get missing lineSpacing ====================
    // After setting lineSpacing on a shape, Get should report it.
    [Fact]
    public void Pptx_Shape_Get_ShouldIncludeLineSpacing()
    {
        using var pptx = new PowerPointHandler(_pptxPath, editable: true);
        ...
        shape.Format.Should().ContainKey("lineSpacing",
            "shape Get should expose lineSpacing property after it's been set");
    }


    // ==================== BUG #6: Word paragraph indentation not in Get for hangingindent ====================
    [Fact]
    public void Word_Paragraph_Get_ShouldIncludeHangingIndent()
    {
        ...
        para.Format.Should().ContainKey("hangingindent",
            "paragraph Get should expose hangingindent when it's set");
    }


    // ==================== BUG #7: Excel cell with formula readback loses formula ====================
    // After setting a formula, Get should return the formula text.
    [Fact]
    public void Excel_Cell_Formula_ShouldBeInGetFormat()
    {
        ...
        cell.Format.Should().ContainKey("formula",
            "cell Get should expose formula when one is set");
        cell.Format["formula"]?.ToString().Should().Be("=SUM(B1:B10)",
            "formula should be readable via Get");
    }


    // ==================== BUG #8: Word header Add with odd font size persistence ====================
    // 15pt → stored as 30 half-points → Get reads 30/2 = 15 (correct, integer division works)
    // But 15.5pt → stored as 31 half-points → Get reads 31/2 = 15 (truncated!)
    [Fact]
    public void Word_Header_FontSize_15_5_Persistence()
    {
        ...
        header.Format["size"]?.ToString().Should().Be("15.5pt",
            "header font size 15.5pt should survive round-trip without truncation");
    }
